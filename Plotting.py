...
# adaptive_moe_df = pd.read_csv("a2cmoe_m3_adaptive_logs.csv")
...
def plot_ci(ax, x, mean, std, label, color):
    ...
    print("********")
    cal_1000 = ((max(mean) - mean[999]) / (max(mean) - min(mean))) * 100
    cal_2000 = ((max(mean) - mean[1999]) / (max(mean) - min(mean))) * 100
    cal_3000 = ((max(mean) - mean[2999]) / (max(mean) - min(mean))) * 100
    cal_4000 = ((max(mean) - mean[3999]) / (max(mean) - min(mean))) * 100
    print(round(cal_1000,2), round(cal_2000,2), round(cal_3000,2), round(cal_4000,2))
    std  = np.asarray(std)
    ax.plot(x, mean, label=label, color=color)
    ax.fill_between(x, mean - std, mean + std, alpha=0.2, color=color)

fig, axs = plt.subplots(1, 3, figsize=(16, 6))
plt.suptitle("Comparison from CSV logs (mean ± std)")
...
axs[0].set_title("Reward Comparison (mean ± std)")
axs[0].legend()
axs[0].grid(True)
...
plot_ci(axs[1], moe["Episode"],  moe["CriticLoss_mean"],  moe["CriticLoss_std"],
        "MoE", "tab:green")
# plot_ci(axs[0], adaptive_moe["Episode"], adaptive_moe["CriticLoss_mean"], adaptive_moe["CriticLoss_std"],
#         "Adaptive MOE", "tab:red")
...
axs[1].set_title("Critic Loss Comparison (mean ± std)")
axs[1].legend()
axs[1].grid(True)
...
plot_ci(axs[2], moe["Episode"],  moe["ActorLoss_mean"],  moe["ActorLoss_std"],
        "MoE", "tab:green")
# plot_ci(axs[0], adaptive_moe["Episode"], adaptive_moe["ActorLoss_mean"], adaptive_moe["ActorLoss_std"],
#         "Adaptive MOE", "tab:red")
...
axs[2].set_title("Actor Loss Comparison (mean ± std)")
axs[2].legend()
axs[2].grid(True)
